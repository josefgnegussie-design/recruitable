...
// Säkerhetsnotis till kontots egen ägare när lösenordet bytts (inte en
// förfrågan/lead-mejl till ett bolag — helt separat från pausen ovan).
// Misslyckas tyst om RESEND_API_KEY saknas, precis som förfrågningsmejlen.
export async function sendPasswordChangedEmail(email) {
  if (!process.env.RESEND_API_KEY) {
    console.error("RESEND_API_KEY saknas — hoppar över lösenordsbekräftelse.");
    return null;
  }
  ...
  const result = await resend.emails.send({
    ...
    text: "Lösenordet för ditt konto på recruitable.se har precis bytts.\n\nVar det inte du som gjorde det? Kontakta oss omgående på info@recruitable.se.\n\n— Recruitable",
    html: `
      <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;max-width:480px;margin:0 auto;color:#16211f;">
        <div style="background:#0f2229;padding:20px 28px;border-radius:10px 10px 0 0;">
          <span style="color:#f2efe9;font-size:18px;font-weight:600;">Recruitable</span>
        </div>
        <div style="border:1px solid #ddd9d1;border-top:none;border-radius:0 0 10px 10px;padding:28px;">
          <p style="font-size:15px;line-height:1.6;margin:0 0 16px;">Lösenordet för ditt konto på recruitable.se har precis bytts.</p>
          <p style="font-size:13px;color:#828b89;line-height:1.6;margin:0;">
            Var det inte du som gjorde det? Kontakta oss omgående på
            <a href="mailto:info@recruitable.se" style="color:#828b89;">info@recruitable.se</a>.
          </p>
        </div>
      </div>`,
  });
  ...
  if (result?.error) {
    console.error("Kunde inte skicka lösenordsbekräftelse:", JSON.stringify(result.error));
  }
