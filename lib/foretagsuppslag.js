...
async function hamtaFranBolagsverket(orgnr) {
  ...
  return {
    ...
    // Datumet, och inte bara flaggan: registerkontrollen i
    // /api/cron/bolagsverket-koll skriver ner det, och "avregistrerat 2025-03-14"
    // är ett annat besked till den som söker efter bolaget än "avregistrerat".
    avregistreringsdatum: org.avregistreradOrganisation?.fel
      ? null
      : org.avregistreradOrganisation?.avregistreringsdatum || null,
...
/* ------------------------------------------------- registerkontroll (cron) */

// Bolagets läge i Bolagsverkets register: lever det, och när avregistrerades det?
//
// Egen väg in, skild från slaUppOrgnr ovan, av tre skäl: registerkontrollen
// frågar bara Bolagsverket (allabolag skrapas inte i ett jobb som går igenom
// tusentals bolag), den vill ha avregistreringsdatumet och inte bara flaggan, och
// den ska inte fylla minnescachen med bolag ingen tittar på just nu.
//
// Kastar aldrig. Ett fel ska inte få jobbet att sluta gå vidare till nästa bolag,
// och framför allt aldrig tolkas som "bolaget finns inte" — då hade ett avbrott
// hos Bolagsverket sett ut som en massavregistrering.
export async function hamtaRegisterstatus(rawOrgnr) {
  const orgnr = normalizeOrgnr(rawOrgnr);
  if (orgnr.length !== 10) return { status: "ogiltigt-orgnr" };

  // Tre försök med växande paus. Ett svep över registret möter förr eller senare
  // ett "fetch failed" som inte har med bolaget att göra — vid första svepet
  // 2026-09-13 föll sex uppslag på nätverksglapp, och alla sex svarade HTTP 200
  // när de provades om en stund senare. Utan omförsöken hade en blinkning fått
  // körningen att stanna, eller ett fullt levande bolag att bokföras som okänt.
  let sisteFel = null;

  for (let forsok = 0; forsok < 3; forsok++) {
    if (forsok > 0) await new Promise((r) => setTimeout(r, forsok * 1000));

    try {
      const bv = await hamtaFranBolagsverket(orgnr);
      if (!bv) return { status: "ej-hittad" };

      return {
        status: "ok",
        namn: bv.namn,
        aktiv: bv.aktiv,
        avregistreringsdatum: bv.avregistreringsdatum,
      };
    } catch (err) {
      sisteFel = err.message;
    }
  }

  return { status: "fel", fel: sisteFel };
}
