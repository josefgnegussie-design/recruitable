...
export function filterCompanies(companies, filters = {}) {
  const { omrade, service, ort, requireKa, requireAuktorisation } = filters;
  return companies.filter((c) => {
    ...
    if (requireKa && !c.ka) return false;
    if (requireAuktorisation && (!c.auktorisation || c.auktorisation.length === 0)) return false;
    return true;
  });
}
